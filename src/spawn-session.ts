...
/**
 * Same schema drift as `currentAgentOf`, plus a field-name mismatch: a
 * session's own `model` (confirmed live via `GET /session/{id}`) comes back
 * as `{ id, providerID, variant }`, while everywhere else in this API a
 * model is `{ providerID, modelID }` (e.g. the prompt body, and a message's
 * own recorded `model`). `variant` of `"default"` is the server's sentinel
 * for "no variant chosen" (confirmed by the server's own diffing check:
 * `he.model?.variant==="default" ? void 0 : he.model?.variant`), so it is
 * dropped here rather than round-tripped as a literal variant name.
 */
function currentModelOf(session: object): DeliveryModel | undefined {
	const model = (session as { model?: { id?: string; providerID?: string; variant?: string } }).model
